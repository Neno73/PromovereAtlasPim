...
*Generated by thoughtful-dev - Last updated: 2025-12-05 14:30*

## System Status ✅

**OPERATIONAL** as of 2025-12-05:
- ✅ Backend running (http://localhost:1337)
- ✅ Frontend running (http://localhost:3000+) - Default 3000, auto-increments if port occupied
- ✅ Database connected (PostgreSQL via Neon) - SSL issue fixed
- ✅ Redis connected (Upstash)
- ✅ Queue system active (5 BullMQ workers)
- ✅ All TypeScript compilation errors resolved
- ✅ Product → ProductVariant hierarchy fully functional
- ✅ **Gemini RAG Integration complete** - FileSearchStore live and verified working
- ✅ **Sync Lock Service** - Distributed locking prevents duplicate syncs
- ✅ **Gemini File Tracking** - `gemini_file_uri` tracks sync status per product

**Recent Update (2025-12-05)**: Fixed Gemini FileSearchStore integration. Files ARE being uploaded correctly (verified via semantic search). Fixed `getStats()` and `deleteDocument()` to use correct APIs. Added `gemini_file_uri` tracking field. See DECISIONS.md for details.

## Quick Context

PromoAtlas is a Strapi 5-based Product Information Management (PIM) system for managing promotional products from Promidata suppliers. The system features automated product synchronization with hash-based incremental updates, multilingual support (NL/DE/EN/FR), Cloudflare R2 image storage, and **Google Gemini RAG integration** for semantic search and AI-powered product discovery.

**Key Architecture**: Two-level product hierarchy (Product → Product Variants) for proper variant management (sizes/colors) with reduced data duplication. **Headless RAG**: Gemini FileSearchStore for semantic embeddings, Meilisearch for exact search, Strapi as source of truth.
...
```bash
cd frontend
npm run dev         # Start Vite development server (default port 3000, auto-increments if occupied)
...
- **Frontend Dev**: http://localhost:3000 (or next available port if 3000 is occupied)
...
- **Auto-sync to Gemini**: Products automatically uploaded to FileSearchStore on update
- **Sync Lock Service**: Redis-based distributed locks prevent duplicate syncs, graceful stop mechanism

### Sync Lock Service (Added 2025-11-27)
- **Distributed locking**: Redis NX SET prevents concurrent syncs for same supplier
- **Graceful stop**: Workers check stop signals between batches, complete current work before stopping
- **Auto-recovery**: 1 hour TTL auto-releases orphaned locks
- **Admin UI**: Sync button toggles to "Stop" while running, polls for status every 5 seconds
- **API endpoints**: `/api/promidata-sync/active`, `/api/promidata-sync/stop/:supplierId`
- **Upstash compatible**: Uses SCAN instead of KEYS for pattern matching

### Gemini RAG Integration (Updated 2025-12-05)
- **FileSearchStore**: Persistent embedding storage for semantic search (separate namespace from Files API)
- **Store Discovery**: Finds store by `displayName = 'PromoAtlas Product Catalog'`
- **Store ID**: `fileSearchStores/promoatlas-product-catalog-xfex8hxfyifx`
- **Sync Tracking**: `gemini_file_uri` field on Product tracks sync status
- **Admin endpoints**: Manual sync control via `/api/gemini-sync/*`
- **Auto-sync**: Integrated with promidata-sync for real-time updates
- **Verified Working**: Semantic search returns real products (A407-2030, A407-2031 chewing gum)

**Important Limitations**:
- `files.list()` does NOT show FileSearchStore files (separate namespace)
- Individual file deletion NOT supported (only entire store deletion)
- Files accumulate but semantic search still works correctly
...
- **Gemini File URI**: Tracks document reference in FileSearchStore (private field)
