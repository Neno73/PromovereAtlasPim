...
import { createExtendedService } from "../../../utils/api-factory";
import crypto from "crypto";

// Use node-fetch for API calls
import fetch from "node-fetch";

export default createExtendedService(
  ...
  ({ strapi }) => ({
    ...
    async fetchCategoriesFromPromidata() {
      try {
        const response = await fetch(
          `${this.promidataConfig.baseUrl}${this.promidataConfig.endpoints.categories}`
        );
        if (!response.ok) {
    ...
    async syncSupplier(supplier: any) {
      try {
        ...
        strapi.log.info(
          `Products to process: ${productsToProcess.map((p) => p.url.split("/").pop()).join(", ")}`
        );
          ...
          try {
            ...
            if (existingProduct && existingProduct.promidata_hash) {
              ...
              if (existingHash === newHash) {
                skipped++;
                strapi.log.info(
                  `✓ Skipping ${productCode} - hash unchanged: ${newHash}`
                );
                continue;
              } else {
                strapi.log.info(
                  `⚡ Processing ${productCode} - hash changed: ${existingHash} → ${newHash}`
                );
              }
            } else {
              // New product or no existing hash
              strapi.log.info(
                `🆕 Processing ${productCode} - ${existingProduct ? "missing hash" : "new product"}: ${newHash}`
              );
            }
            ...
            strapi.log.info(
              `Product data fetched, creating/updating product...`
            );

            // Create or update product
            const result = await this.createOrUpdateProduct(
              productData,
              supplier,
              hash,
              url
            ...
            if (result.created) {
              imported++;
              strapi.log.info(
                `✅ Product ${productCode} created with hash: ${hash}`
              );
            } else {
              updated++;
              strapi.log.info(
                `🔄 Product ${productCode} updated with hash: ${hash}`
              );
            }
          } catch (error) {
            const urlParts = url.split("/");
        ...
        const efficiencyPercent =
          totalAvailable > 0 ? Math.round((skipped / totalAvailable) * 100) : 0;

        strapi.log.info(
          `Sync completed for supplier: ${supplier.code} - Imported: ${imported}, Updated: ${updated}, Skipped: ${skipped}, Errors: ${errors.length}`
        );
        strapi.log.info(
          `Incremental sync efficiency: ${efficiencyPercent}% skipped (${skipped}/${totalAvailable} unchanged)`
        );
    ...
    async createOrUpdateProduct(
      ...
      try {
        ...
        const nameJson: any = {};
        if (
          productData.ChildProducts &&
          productData.ChildProducts[0] &&
          productData.ChildProducts[0].ProductDetails
        ) {
        ...
        if (
          ...
          const details = productData.ChildProducts[0].ProductDetails;
          if (details.en && details.en.Description)
            descriptionJson.en = details.en.Description;
          if (details.de && details.de.Description)
            descriptionJson.de = details.de.Description;
          if (details.fr && details.fr.Description)
            descriptionJson.fr = details.fr.Description;
          if (details.nl && details.nl.Description)
            descriptionJson.nl = details.nl.Description;
        }
        ...
        if (
          productData.ChildProducts &&
          productData.ChildProducts[0] &&
          productData.ChildProducts[0].ProductPriceCountryBased
        ) {
          const priceData =
            productData.ChildProducts[0].ProductPriceCountryBased;
        ...
        if (productData.ChildProducts && productData.ChildProducts[0]) {
          ...
          if (childProduct.ProductDetails) {
            Object.keys(childProduct.ProductDetails).forEach((lang) => {
              ...
              if (langDetails.ConfigurationFields) {
                const colorField = langDetails.ConfigurationFields.find(
                  (field: any) => field.ConfigurationName === "Color"
                );
              ...
              if (
                langDetails.UnstructuredInformation &&
                langDetails.UnstructuredInformation.PMSValue
              ) {
          ...
          if (
            childProduct.NonLanguageDependedProductDetails &&
            childProduct.NonLanguageDependedProductDetails.SearchColor
          ) {
            searchColor =
              childProduct.NonLanguageDependedProductDetails.SearchColor;
          }
        ...
        if (
          ...
          Object.keys(details).forEach((lang) => {
            ...
            if (
              langDetails.WebShopInformation &&
              langDetails.WebShopInformation.Material &&
              langDetails.WebShopInformation.Material.InformationValue
            ) {
              materialJson[lang] =
                langDetails.WebShopInformation.Material.InformationValue;
            }
        ...
        if (
          productData.ChildProducts &&
          productData.ChildProducts[0] &&
          productData.ChildProducts[0].NonLanguageDependedProductDetails
        ) {
          const details =
            productData.ChildProducts[0].NonLanguageDependedProductDetails;
          if (details.Weight) {
        ...
        if (
          ...
          if (
            details.DimensionsLength ||
            details.DimensionsWidth ||
            details.DimensionsHeight
          ) {
        ...
        try {
          // Upload main image
          if (mainImageUrl) {
            mainImageId = await this.uploadImageFromUrl(
              mainImageUrl,
              `${productCode}-main`
            );
          }
          ...
          for (let i = 0; i < galleryUrlsToProcess.length; i++) {
            try {
              const galleryImageId = await this.uploadImageFromUrl(
                galleryUrlsToProcess[i],
                `${productCode}-gallery-${i + 1}`
              );
              if (galleryImageId) {
              ...
              strapi.log.warn(
                `Failed to upload gallery image ${i + 1} for ${productCode}:`,
                error.message
              );
            }
          }
          ...
          strapi.log.warn(
            `Image upload failed for ${productCode}:`,
            error.message
          );
        }
        ...
        strapi.log.error(
          `Failed to create/update product ${this.extractProductCode(productData, supplier, url)}:`,
          error
        );
        throw error;
      }
