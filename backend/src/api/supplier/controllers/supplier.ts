...
import { createExtendedController } from "../../../utils/api-factory";

export default createExtendedController(
    ...
    async syncA23(ctx) {
      try {
        ...
        if (existingProducts.length > 0) {
          ...
          strapi.log.info(
            `Deleted product ${existingProducts[0].id} for fresh import test`
          );
        }
        ...
        ctx.body = {
          message:
            "A23 sync completed successfully with enhanced fields (FORCED UPDATE)",
          supplier: supplier.code,
          enhanced_fields: [
            "images (main + gallery)",
      ...
      try {
        ...
        ctx.body = {
          ...
          sample_product: {
            ...
            price_tiers_count: (product as any).price_tiers
              ? (product as any).price_tiers.length
              : 0,
            weight: product.weight || "MISSING",
            dimension: product.dimension || "MISSING",
            name_languages: product.name ? Object.keys(product.name) : [],
            description_languages: product.description
              ? Object.keys(product.description)
              : [],
            material_languages: product.material
              ? Object.keys(product.material)
              : [],
            color_name_languages: product.color_name
              ? Object.keys(product.color_name)
              : [],
          ...
          field_extraction_summary: {
            ...
            multilingual_content:
              product.name && Object.keys(product.name).length > 0
                ? "✅ Collected"
                : "❌ Missing",
            brand_info: product.brand ? "✅ Collected" : "❌ Missing",
            color_info:
              product.search_color || product.color_code
                ? "✅ Collected"
                : "❌ Missing",
            pricing:
              (product as any).price_tiers &&
              (product as any).price_tiers.length > 0
                ? "✅ Collected"
                : "❌ Missing",
            geographic: product.country_of_origin
              ? "✅ Collected"
              : "❌ Missing",
          },
        };
