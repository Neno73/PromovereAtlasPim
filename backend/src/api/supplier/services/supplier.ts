...
export default createExtendedService(
  ...
  ({ strapi }) => ({
    async bootstrap() {
      ...
      const existingSuppliers = await strapi.entityService.findMany(
        "api::supplier.supplier",
        {
          fields: ["code"],
        }
      ...
      const existingCodes = existingSuppliers.map((s: any) => s.code);

      // Create missing suppliers
      for (const supplier of suppliers) {
        if (!existingCodes.includes(supplier.code)) {
          const newSupplier = await strapi.entityService.create(
            "api::supplier.supplier",
            {
              data: {
                code: supplier.code,
